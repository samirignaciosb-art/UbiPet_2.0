...
// FIX: vincularPlacaEmpty ahora guarda también el placa_id en sessionStorage
// para que crearMascota pueda vincular automáticamente sin que el usuario
// tenga que presionar "Vincular" manualmente después.
async function vincularPlacaEmpty() {
  ...
  // Guardar tanto el código como el id para vinculación automática en crearMascota
  sessionStorage.setItem('placa_pendiente', codigo)
  sessionStorage.setItem('placa_pendiente_id', placa.id)
  window._nuevaMascota()
}

// FIX: crearMascota ahora vincula la placa automáticamente si viene del flujo
// empty state, sin requerir un segundo paso manual del usuario.
async function crearMascota() {
  ...
  if (croppedBlob) {
    try {
      const url = await subirFoto(croppedBlob, data.id)
      await supabase.from('perfiles').update({ foto_url: url }).eq('id', data.id)
      mascotaActual.foto_url = url
    } catch (e) {}
  }

  // Vincular placa automáticamente si venía del flujo empty state
  const placaId = sessionStorage.getItem('placa_pendiente_id')
  let placaVinculada = false
  if (placaId) {
    const { error: e1 } = await supabase.from('perfiles').update({ placa_id: placaId }).eq('id', data.id).eq('user_id', state.user.id)
    if (!e1) {
      await supabase.from('placas').update({ estado: 'activa', perfil_id: data.id }).eq('id', placaId)
      sessionStorage.removeItem('placa_pendiente')
      sessionStorage.removeItem('placa_pendiente_id')
      placaVinculada = true
    }
  }
  ...
  showToast(placaVinculada ? '✅ ¡Mascota creada y placa vinculada!' : '✅ ¡Mascota creada!')
}
...
// FIX: asociarPlaca simplificado — se eliminó el setTimeout y la llamada doble
// a renderPlacaSection (renderDetalle ya la llama internamente).
async function asociarPlaca() {
  ...
  if (m) { mascotaActual = m; renderDetalle() }
}
...
function generarBlob() {
  ...
  out.toBlob(async blob => {
    ...
    if (!modoNueva && mascotaActual?.id) {
      try {
        const fu = await subirFoto(blob, mascotaActual.id)
        await supabase.from('perfiles').update({ foto_url: fu }).eq('id', mascotaActual.id)
        mascotaActual.foto_url = fu
        if (av) { const img2 = av.querySelector('img'); if (img2) img2.src = fu }
        await cargarMascotas(state.user)
